...
CONFIG_PATH = Path(os.environ.get("FORGEMEM_CONFIG", Path.home() / ".forgememo" / "config.json"))
...
MAX_CONFIG_SIZE = 1_000_000  # 1 MB — anything larger is certainly corrupt
...
def save(cfg: dict) -> None:
    ...
    # power loss or concurrent writes.
    try:
        ...
        # Last resort: direct write (e.g. filesystem doesn't support rename)
        CONFIG_PATH.write_text(json.dumps(cfg, indent=2))


def get_provider() -> str:
    provider = load().get("provider", "anthropic")
...
def get_api_key(provider: str) -> str | None:
    ...
    if isinstance(api_keys, dict):
        key_in_config = api_keys.get(provider)
        if key_in_config and isinstance(key_in_config, str) and key_in_config.strip():

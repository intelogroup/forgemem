...
LOG_FILE = FORGEMEM_DIR / "forgememo_daemon.log"
...
def main():
    ...
    if _check_port("127.0.0.1", 5555):
        logger.error(
            "Port 5555 already in use — another instance may be running. Exiting."
        )
        sys.exit(1)
    ...
    try:
        ...
        app.run(
            host="127.0.0.1", port=5555, debug=False, threaded=True, use_reloader=False
        )

...
class TestFingerprintCaseNormalization:
    ...
    def test_mixed_path_separators_same_fingerprint(self):
        ...
        # Both should strip paths, leaving just "TypeError: bad at"
...
class TestKeywordCaseNormalization:
    def test_posix_paths_stripped_from_keywords(self):
        kw = _extract_error_keywords("Error in /Users/alice/deep/path/foo.py: bad input")
    ...
    def test_windows_paths_stripped_from_keywords(self):
        kw = _extract_error_keywords("Error in C:\\Users\\bob\\project\\main.py: bad input")
        assert "Users" not in kw
    ...
    def test_keywords_are_meaningful(self):
        ...
        words = kw.split()
        assert any(w in ("TypeError", "Cannot", "read", "property", "null") for w in words)
...
class TestProjectIdCasing:
    def _insert_summary(self, project_id, title, narrative):
        ...
        conn.execute(
            ...
            "VALUES (?,?,?,?,?,?)",
            (row_id, title, narrative, "case-test", "", project_id),
        )
        conn.commit()
        conn.close()
        return row_id
...
class TestErrorEventsCaseSensitivity:
    def test_session_id_is_case_sensitive(self, client):
        ...
        assert r.get_json()["count"] == 0  # Different case = different session
    ...
    def test_fingerprint_is_case_sensitive(self, client):
        ...
        # Fingerprints are generated as lowercase hex, but DB comparison is exact

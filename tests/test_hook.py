...
class TestIsSessionStart:
    """_is_session_start detects first-prompt vs mid-session via transcript."""

    def test_no_transcript_path_is_session_start(self):
        from forgememo.hook import _is_session_start
        assert _is_session_start({}) is True

    def test_missing_file_treated_as_session_start(self, tmp_path):
        from forgememo.hook import _is_session_start
        assert _is_session_start({"transcript_path": str(tmp_path / "no.json")}) is True

    def test_empty_transcript_is_session_start(self, tmp_path):
        import json
        from forgememo.hook import _is_session_start
        f = tmp_path / "t.json"
        f.write_text(json.dumps([]))
        assert _is_session_start({"transcript_path": str(f)}) is True

    def test_only_user_messages_is_session_start(self, tmp_path):
        import json
        from forgememo.hook import _is_session_start
        f = tmp_path / "t.json"
    ...
    def test_assistant_turn_present_is_not_session_start(self, tmp_path):
        import json
        from forgememo.hook import _is_session_start
        f = tmp_path / "t.json"
        f.write_text(json.dumps([
        ...
        assert _is_session_start({"transcript_path": str(f)}) is False

    def test_messages_key_format(self, tmp_path):
        import json
        from forgememo.hook import _is_session_start
        f = tmp_path / "t.json"
